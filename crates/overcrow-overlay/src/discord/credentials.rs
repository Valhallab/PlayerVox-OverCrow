...
const KEYRING_USER: &str = "discord-oauth-v1";
...
impl DiscordCredentials {
    pub fn new(
        ...
        let mut access_token = access_token.into();
        let mut refresh_token = refresh_token.into();
...
impl fmt::Debug for DiscordCredentials {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("DiscordCredentials")
...
impl Drop for DiscordCredentials {
    fn drop(&mut self) {
...
impl fmt::Display for CredentialStoreError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Corrupt => formatter.write_str("stored Discord credential is invalid"),
        }
    }
}
...
pub fn decode_credentials(bytes: &[u8]) -> Result<DiscordCredentials, CredentialStoreError> {
